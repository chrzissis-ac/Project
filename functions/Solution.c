...
void createGen_sol(Gen_eigensol * eigensol,Gen_sol ** GSol){
	...
	for(i=0;i < eigensol->dim;i++){
			flag=0;
			for(k=0;k<j;k++){
				if(eigensol->solution[i].eigenvalue==(*GSol)->solution[k].value[a]){
					flag=1;
					(*GSol)->solution[k].multiplicity=(*GSol)->solution[k].multiplicity+1;
					if((*GSol)->solution[k].inf==0){(*GSol)->solution[k].inf=-2;}
				}
			}
			if(flag==0){
...
static void createSolution(Gen_eigensol ** solution, int dim, int grade, double * realSolution, double * imaginarySolution,double * betaSolution, double * eigenVector,char c) {
	...
	for (i=0 ; i<dim ; i++) {
		//printf("Vector --- %d eigenvalue= %f\n",i,temp2[i]);
		//printf("Einai %f\n",temp1[i*dim+dim/grade-1]);
		if (temp1[i*dim+dim/grade-1]!=0.0 && (*solution)->solution[i].inf==0.0) {
			//printf("mpainw sto 1\n");
			...
			for (j=0 ; j<grade ; j++) {
				...
				for (k=0 ; k<dim/grade ; k++){
					...
					//printf("|%f| position = %d\n",temp1[i*dim+j*dim/grade+k],i*dim+j*dim/grade+k);
					//temp1[i*dim+j]=temp1[i*dim/grade+j]/temp1[(i+1)*dim/grade-1];
				}
			a=a*temp2[i];
			}
		}
